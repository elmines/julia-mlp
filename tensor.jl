...
struct Operation{N} <: Tensor{N}
	...
	Operation(parents::Vector{<:Tensor}, size::SizeType, callback::Function, grad_callbacks::Vector{Function}) = new{length(size)}(getNewId(), size, parents, callback, grad_callbacks)
end
